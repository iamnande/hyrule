...
`make new-service` (`local/new-service.sh`) prompts for a slug, a name, a
...
every new service gets `go/cmd/<slug>/{main.go,app/module.go}`,
`go/internal/svc/<slug>/module.go` (an empty `fx.Module` - it boots, serves
...
starting content to put there. `go/internal/svc/pings` is the reference for
adding a first endpoint the same way pings got one: a path in
`openapi.yaml`, oapi-codegen wired into `go/internal/svc/<slug>/api`, a
domain type, and (if it owns a database) a migration plus a `sql:` entry
in `sqlc.yaml`.

## database config moved out of go/internal/lib/config

`go/internal/lib/config.PingsModule` used to bundle `LoadDatabase()` in with
...
touches the Tiltfile. the dev postgres (`local/cluster/postgres.yaml`) is
